...
def get_augmenter():
    ...
    def augment(images):
        blurred_imgs = []
        # for bt in images:
        trans = T.GaussianBlur(kernel_size=(3, 3), sigma=(3, 7))
        # print(type(trans))
        blurred_imgs.append(trans(images))
        # print(type(blurred_imgs[0]))
        img = torch.cat(blurred_imgs)
        # print(img.shape)
        return img
    return augment
...
def bi_train(model, label_loader, unlabeled_loader, val_loader, test_loader, optimizer, ssl_obj):
    ...
    for l_data, u_data in zip(label_loader, unlabeled_loader):
        ...
        l_images = augment_fn(l_images)
        u_images = augment_fn(u_images)
        if args.dataset == 'MNIST':
